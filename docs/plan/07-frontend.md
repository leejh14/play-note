# 프론트엔드 구현 계획 (Next.js)

> 모바일 우선 웹앱. App Router + Apollo Client + Tailwind CSS.
> 주 사용 환경: 카카오톡 링크 → 모바일 브라우저.

---

## 1. 기술 스택

| 분류 | 선택 |
|------|------|
| 프레임워크 | Next.js (App Router) |
| GraphQL Client | Apollo Client |
| 스타일링 | Tailwind CSS |
| 아이콘 | Lucide React |
| 카카오 공유 | Kakao JS SDK |
| 상태 관리 | Apollo Cache (추가 상태 라이브러리 불필요) |

## 1-A. Phase 07 구현 결정 (확정값)

- 인증 정책은 API 현재 상태와 동일하게 유지:
  - 공개 API: `sessionPreview`(query), `createSession`(mutation)
  - `/sessions`, `/stats`, `/friends`, `/s/{id}/setup`, `/s/{id}/detail`는 토큰 필요
- 세션 ID 정책:
  - URL 경로: Global ID (`/s/{sessionGlobalId}`)
  - 인증 헤더: Local ID (`x-session-id`)
  - 클라이언트에서 Global↔Local 변환
- 토큰 저장:
  - 기본 토큰: `playnote:session:{localSessionId}:token`
  - 공유용 토큰(선택): `playnote:session:{localSessionId}:share-token`
  - 활성 세션: `playnote:active-session-id`
- role probe 추가:
  - Query: `authContext`
  - 응답: `{ sessionId, role }`
  - `role`이 `ADMIN`일 때만 Friend CRUD / Match 삭제 UI 노출
- 무토큰 접근 UX:
  - 링크 유도 화면 노출("공유 링크로 먼저 입장")

---

## 2. 라우팅 구조

```
src/app/
├── layout.tsx                       # 루트 레이아웃 (Apollo Provider, 탭 바)
├── page.tsx                         # redirect → /sessions
│
├── sessions/
│   ├── page.tsx                     # 세션 목록 (Tab: Sessions)
│   └── new/
│       └── page.tsx                 # 세션 생성
│
├── s/[sessionId]/
│   ├── page.tsx                     # 세션 진입점 (토큰 처리 + 라우팅)
│   ├── setup/
│   │   └── page.tsx                 # 세션 셋업 (scheduled 상태)
│   └── detail/
│       └── page.tsx                 # 세션 상세 (confirmed/done 상태)
│
├── stats/
│   ├── page.tsx                     # 통계 개요 (Tab: Statistics)
│   └── [friendId]/
│       └── page.tsx                 # 친구별 상세 통계
│
└── friends/
    └── page.tsx                     # 친구 관리 (Tab: Friends)
```

### 페이지별 역할

| 경로 | 역할 | 토큰 | 탭 바 |
|------|------|------|-------|
| `/sessions` | 세션 목록 (홈) | 필요 (active session token) | O |
| `/sessions/new` | 세션 생성 | 불필요 (`createSession` 공개) | X (뒤로가기) |
| `/s/{id}` | 진입점: 토큰 저장 → 상태별 리다이렉트 | URL 파라미터 `?t=` | X |
| `/s/{id}/setup` | 셋업 (참가/팀/라인) | 필요 | X (뒤로가기) |
| `/s/{id}/detail` | 상세 (매치/첨부/댓글) | 필요 | X (뒤로가기) |
| `/stats` | 통계 개요 | 필요 (active session token) | O |
| `/stats/{friendId}` | 친구별 상세 통계 | 필요 (active session token) | X (뒤로가기) |
| `/friends` | 친구 관리 | 필요 (admin) | O |

---

## 3. 토큰 관리

### 저장

```typescript
// lib/token.ts
const TOKEN_PREFIX = 'playnote:session';

function saveToken(sessionId: string, token: string) {
  localStorage.setItem(`${TOKEN_PREFIX}:${sessionId}:token`, token);
}

function getToken(sessionId: string): string | null {
  return localStorage.getItem(`${TOKEN_PREFIX}:${sessionId}:token`);
}

function removeToken(sessionId: string) {
  localStorage.removeItem(`${TOKEN_PREFIX}:${sessionId}:token`);
}
```

### 진입 플로우 (`/s/{sessionId}`)

```
1. URL에서 ?t= 파라미터 추출
2. ?t= 있으면 → localStorage token/share-token 최신값으로 저장(기존 값 덮어쓰기)
3. sessionId로 세션 상태 조회 (GraphQL)
4. status에 따라 라우팅:
   - scheduled → /s/{id}/setup
   - confirmed / done → /s/{id}/detail
5. `INVALID_TOKEN | SESSION_NOT_FOUND | UNAUTHORIZED` 발생 시:
   - `playnote:session:{id}:token`, `share-token` 제거
   - `playnote:active-session-id`가 해당 세션이면 제거
   - 재진입 안내 UI 노출
```

### Apollo Client 헤더 주입

```typescript
const authLink = setContext((operation, { headers }) => {
  // GraphQL operation의 variables에서 sessionId 추출
  // 또는 현재 URL path에서 sessionId 추출
  const sessionId = extractSessionId(operation, window.location);
  const token = sessionId ? getToken(sessionId) : null;

  return {
    headers: {
      ...headers,
      ...(sessionId && { 'x-session-id': sessionId }),
      ...(token && { 'x-session-token': token }),
    },
  };
});
```

---

## 4. 화면 구성

### 4.1 하단 탭 바 (Global Navigation)

```
┌─────────────────────────────┐
│                             │
│       (Page Content)        │
│                             │
├─────────┬─────────┬─────────┤
│ Sessions│ Stats   │ Friends │
│ 📅      │ 📊      │ 👥      │
└─────────┴─────────┴─────────┘
```

- 활성 탭: `$primary` (#2563EB) + fontWeight 600
- 비활성 탭: `$gray-400` + fontWeight 500
- 하위 페이지(세션 상세, 통계 상세)에서는 탭 바 숨김 + 뒤로가기 화살표

### 4.2 세션 목록 (`/sessions`)

```
┌─────────────────────────────┐
│ PlayNote             [+ 생성]│
├─────────────────────────────┤
│ [All] [LoL] [Futsal]        │  ← 컨텐츠 필터 탭
├─────────────────────────────┤
│ ┌─────────────────────────┐ │
│ │ 🟢 롤 내전              │ │  ← status badge
│ │ 2026.03.07 (토) 19:00   │ │
│ │ 참가 7/10  매치 2       │ │
│ └─────────────────────────┘ │
│ ┌─────────────────────────┐ │
│ │ 🟡 풋살                 │ │
│ │ 2026.03.10 (월) 18:00   │ │
│ │ 참가 4/8                │ │
│ └─────────────────────────┘ │
│        (더 보기...)         │
└─────────────────────────────┘
```

- Relay cursor pagination (무한 스크롤 또는 "더 보기")
- 기본 정렬: 오늘과 가까운 순 → 활성 상태 우선

### 4.3 세션 생성 (`/sessions/new`)

```
┌─────────────────────────────┐
│ ← 새 세션                   │
├─────────────────────────────┤
│                             │
│ 컨텐츠 선택                  │
│ ┌──────────┐ ┌──────────┐  │
│ │   LoL    │ │  풋살    │  │
│ └──────────┘ └──────────┘  │
│                             │
│ 제목 (선택)                  │
│ ┌─────────────────────────┐ │
│ │                         │ │
│ └─────────────────────────┘ │
│                             │
│ 날짜 / 시간                  │
│ ┌─────────────────────────┐ │
│ │ 2026.03.07 (토) 19:00   │ │
│ └─────────────────────────┘ │
│                             │
│ ┌─────────────────────────┐ │
│ │     세션 생성하기         │ │
│ └─────────────────────────┘ │
└─────────────────────────────┘
```

- 생성 완료 → 카카오톡 공유 / 링크 복사 화면
- editorToken, adminToken 반환

### 4.4 세션 셋업 (`/s/{id}/setup`)

```
┌─────────────────────────────┐
│ ← 셋업   롤 내전  [공유] [⚙]│
├─────────────────────────────┤
│ 참가 상태                    │
│ ┌───────────────────┬───┐  │
│ │ 🟢 철수           │ ✓ │  │  ← ATTENDING
│ │ ⚪ 영희           │ ? │  │  ← UNDECIDED
│ │ 🔴 민수           │ ✗ │  │  ← NOT_ATTENDING
│ └───────────────────┴───┘  │
│                             │
│ 팀 배정                      │
│ Team A          Team B      │
│ ┌──────────┐ ┌──────────┐  │
│ │ 철수 TOP │ │ 영수 JG  │  │
│ │ 지수 MID │ │ 현우 ADC │  │
│ │ ...      │ │ ...      │  │
│ └──────────┘ └──────────┘  │
│                             │
│ ┌─────────────────────────┐ │
│ │     셋업 확정하기         │ │
│ └─────────────────────────┘ │
└─────────────────────────────┘
```

### 4.5 세션 상세 (`/s/{id}/detail`)

```
┌─────────────────────────────┐
│ ← 세션 상세     [공유] [⚙] │
├─────────────────────────────┤
│ 롤 내전 — 2026.03.07 19:00 │
│ 🟢 confirmed  🔒 잠김      │
│                             │
│ ─── 팀 구성 ───             │
│ Team A: 철수(TOP) 지수(MID)│
│ Team B: 영수(JG) 현우(ADC) │
│                             │
│ ─── 매치 ───                │
│ ┌─────────────────────────┐ │
│ │ Match #1    🏆 Team A   │ │
│ │ Blue: Team A            │ │
│ │ [End Screen 📷]         │ │  ← LOL_RESULT_SCREEN 업로드
│ │ OCR: ✅ 추출 완료        │ │
│ └─────────────────────────┘ │
│ ┌─────────────────────────┐ │
│ │ Match #2    미확정       │ │
│ │ [+ End Screen 업로드]   │ │
│ └─────────────────────────┘ │
│ [+ 새 매치 생성]            │
│                             │
│ ─── 사진 ───                │
│ [📷] [📷] [📷]  [+ 업로드] │
│ 3/10장                      │
│                             │
│ ─── 댓글 ───                │
│ 철수: 오늘 재밌었다!        │
│ [댓글 입력...]              │
└─────────────────────────────┘
```

### 4.6 통계 개요 (`/stats`)

```
┌─────────────────────────────┐
│ Statistics                  │
├─────────────────────────────┤
│ ┌──────┬────┬─────┬──────┐ │
│ │ 이름 │ WR │ W-L │ Lane │ │
│ ├──────┼────┼─────┼──────┤ │
│ │ 철수 │67%│ 4-2 │ TOP  │ │  ← 탭 → 상세
│ │ 영희 │60%│ 3-2 │ MID  │ │
│ │ 민수 │50%│ 2-2 │ JG   │ │
│ │ 지수 │ – │ 0-0 │  –   │ │  ← 기록 없음
│ └──────┴────┴─────┴──────┘ │
└─────────────────────────────┘
```

### 4.7 통계 상세 (`/stats/{friendId}`)

```
┌─────────────────────────────┐
│ ← 철수                      │
├─────────────────────────────┤
│ ┌────────┬────────┬───────┐ │
│ │ Win 67%│ 6 Games│ TOP   │ │  ← 요약 카드
│ └────────┴────────┴───────┘ │
│                             │
│ ─── 라인 분포 ───           │
│ TOP  ████████████  5        │
│ JG   ████         2        │
│ MID  ██           1        │
│ ADC                0        │
│ SUP                0        │
│                             │
│ ─── 자주 이긴 챔피언 ───    │
│ ┌──────────┬───┬───┬────┐  │
│ │ Champion │ W │ G │ WR │  │
│ │ Garen    │ 3 │ 4 │75% │  │
│ │ Darius   │ 2 │ 2 │100%│  │
│ └──────────┴───┴───┴────┘  │
└─────────────────────────────┘
```

---

## 5. 카카오톡 공유

### SDK 초기화

```typescript
// lib/kakao.ts
declare global { interface Window { Kakao: any } }

export function initKakao() {
  if (typeof window !== 'undefined' && window.Kakao && !window.Kakao.isInitialized()) {
    window.Kakao.init(process.env.NEXT_PUBLIC_KAKAO_JS_KEY);
  }
}
```

### 공유 함수

```typescript
export function shareSession(session: SessionData) {
  const shareUrl = `${window.location.origin}/s/${session.id}?t=${session.editorToken}`;

  window.Kakao.Share.sendDefault({
    objectType: 'feed',
    content: {
      title: `${contentLabel(session.contentType)} — ${formatDate(session.startsAt)}`,
      description: `${formatTime(session.startsAt)} · 참가 ${session.attendingCount}/${session.totalCount}`,
      imageUrl: thumbnailUrl(session.contentType),
      link: { mobileWebUrl: shareUrl, webUrl: shareUrl },
    },
    buttons: [{ title: '참가하기', link: { mobileWebUrl: shareUrl, webUrl: shareUrl } }],
  });
}
```

### 폴백: 링크 복사

```typescript
export async function copyShareLink(sessionId: string, token: string) {
  const url = `${window.location.origin}/s/${sessionId}?t=${token}`;
  await navigator.clipboard.writeText(url);
  // toast: "링크가 복사되었습니다"
}
```

---

## 6. OG 메타 태그

```typescript
// app/s/[sessionId]/layout.tsx
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { sessionId } = params;
  const preview = await fetchSessionPreview(sessionId); // sessionPreview 공개 쿼리

  return {
    title: `${contentLabel(preview.contentType)} — ${formatDate(preview.startsAt)}`,
    description: `${formatTime(preview.startsAt)} · PlayNote`,
    openGraph: {
      title: `${contentLabel(preview.contentType)} — ${formatDate(preview.startsAt)}`,
      description: `${formatTime(preview.startsAt)} · PlayNote`,
      images: [thumbnailUrl(preview.contentType)],
    },
  };
}
```

---

## 7. 컴포넌트 구조

```
src/components/
├── layout/
│   ├── bottom-tab-bar.tsx
│   ├── page-header.tsx          # 뒤로가기 + 제목 + 액션 버튼
│   └── loading-spinner.tsx
├── session/
│   ├── session-card.tsx          # 목록 카드
│   ├── session-status-badge.tsx
│   ├── attendance-list.tsx
│   ├── team-board.tsx            # A/B 팀 드래그앤드롭
│   ├── lane-selector.tsx
│   ├── match-card.tsx
│   ├── match-result-confirm.tsx
│   ├── photo-grid.tsx
│   └── comment-section.tsx
├── friend/
│   ├── friend-list.tsx
│   └── friend-form.tsx
├── stats/
│   ├── stats-table.tsx
│   ├── lane-bar-chart.tsx
│   └── champion-table.tsx
├── upload/
│   ├── image-picker.tsx          # 모바일 갤러리 다중 선택
│   └── upload-progress.tsx
├── share/
│   ├── share-buttons.tsx         # 카카오톡 + 링크 복사
│   └── share-complete.tsx
└── ui/
    ├── button.tsx
    ├── input.tsx
    ├── select.tsx
    ├── dialog.tsx                # 확인 다이얼로그
    ├── toast.tsx
    └── badge.tsx
```

---

## 8. 에러 UX

| 상황 | 대응 |
|------|------|
| 만료/잘못된 토큰 | "접근 권한이 없습니다. 링크를 다시 받아주세요." |
| 삭제된 세션 | "존재하지 않는 세션입니다." |
| 네트워크 에러 | 재시도 버튼 + "연결을 확인해주세요." |
| 10장 초과 | "세션당 최대 10장까지 업로드할 수 있습니다." |
| admin 권한 필요 | "관리자 권한이 필요합니다." |
| effectiveLocked | "사진이 업로드된 후에는 셋업을 변경할 수 없습니다." |

---

## 9. 검증 체크리스트

- [ ] 모바일 브라우저에서 전체 플로우 통과
- [ ] 카카오톡 공유 메시지 정상 렌더링
- [ ] 링크 복사 폴백 동작
- [ ] OG 메타 태그 프리뷰 (슬랙, iMessage 등)
- [ ] 토큰 없이 세션 접근 시 적절한 에러
- [ ] 무한 스크롤 세션 목록
- [ ] 이미지 다중 업로드 (갤러리 선택)
- [ ] 실시간 반영 (Apollo Cache 업데이트)
